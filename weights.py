...
src = "/home/kepecs/Downloads/balbc_w2_pr2_cad_weights_zmd_may-july2021.xlsx"
...
pr2w2 = df["pr2_w2"]
plt.figure()
sns.lineplot(x = "date", y = "tumor length (mm)", hue = "animal name", data = pr2w2)
...
plt.savefig(os.path.join(dst, "pr2w2_tumor_length_plot_{}.jpg".format(date.today().strftime("%Y%m%d"))), bbox_inches = "tight")

#%%
#cadw1
cadw1 = df["cadw1"]
plt.figure()
sns.lineplot(x = "date", y = "% weight", hue = "animal name", data = cadw1)
...
cadw2 = df["cadw2"]
plt.figure()
sns.lineplot(x = "date", y = "\ntumor length (mm)", hue = "animal name", data = cadw2)
...
plt.savefig(os.path.join(dst, "cadw2_tumor_length_plot_{}.jpg".format(date.today().strftime("%Y%m%d"))), bbox_inches = "tight")
